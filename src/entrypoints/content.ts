import type { BookInfo, BookPage, JobOptions, RuntimeMessage, TocBranch, TocItem, TocTree } from '@/lib/shamela/types'
...
const scrape_book = async (job_id: string, book_id: number, options: JobOptions, signal: AbortSignal) => {
  ...
  for (let page_number = start_page; page_number <= end_page; page_number += 1) {
    ...
    const doc = page_number === 1 ? first_page_doc : await fetch_doc(`${base_url}/${page_number}`, signal)
